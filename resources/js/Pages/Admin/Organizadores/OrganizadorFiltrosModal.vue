...
const localFiltros = ref({ taller: "", nombre: "" });
...
const limpiar = () => {
  localFiltros.value = { taller: "", nombre: "" };
  emit("update:filtros", { ...localFiltros.value });
  emit("clear");
};
</script>
...
<template>
  <Modal :show="show" @close="() => emit('close')">
    <div
      class="p-6 w-full mx-auto overflow-x-hidden
      ...
      <form @submit.prevent="aplicar" class="space-y-4">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1">
            Filtra contra los datos del usuario en la API central.
          </p>
        </div>

        <!-- Taller -->
        <div>
          <label for="f-taller" class="block text-sm font-medium text-gray-700"
            >Taller</label
          >
          <select
            id="f-taller"
            v-model="localFiltros.taller"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-azul focus:ring-azul sm:text-sm"
          >
