...
// v-model style for `filtros`
const emit = defineEmits(["close", "update:filtros"]);

// Copia local para editar sin tocar el estado del padre hasta “Aplicar”
const localFiltros = ref({ nombre: "", ciudad: "" });

// Sincronizar al abrir/cambiar
watch(
  () => props.show,
  (open) => {
    if (open) {
...
const limpiar = () => {
  localFiltros.value = { nombre: "", ciudad: "" };
  emit("update:filtros", { ...localFiltros.value });
};
</script>
